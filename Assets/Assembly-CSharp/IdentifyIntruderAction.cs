...
public class IdentifyIntruderAction : GhostAction
{
	...
	public override float CalculateUtility()
	{
		...
		if (_running || (_data.sensor.isPlayerHeldLanternVisible && (_data.threatAwareness > GhostData.ThreatAwareness.EverythingIsNormal || _data.playerLocation.distance < 20f) && _controller.GetNodeMap().CheckLocalPointInBounds(_data.playerLocation.localPosition)) || _data.sensor.isIlluminatedByPlayer)
		{
			return 80f;
		}
		return -100f;
	...
	protected override void OnEnterAction()
	{
		...
		_checkTimer = 0f;
		_effects.PlayVoiceAudioNear((_numTimesIlluminatedByPlayer <= 2) ? AudioType.Ghost_Identify_Curious : AudioType.Ghost_Identify_Irritated);
	}
	...
	public override bool Update_Action()
	{
		...
		{
			_effects.PlayVoiceAudioNear(AudioType.Ghost_Identify_Fail);
			return false;
		}
		return true;
	}
	...
	public override void FixedUpdate_Action()
	{
		...
		bool flag = !_data.lastKnownSensor.isPlayerVisible && _data.lastKnownSensor.isIlluminatedByPlayer && _numTimesIlluminatedByPlayer > 2;
		...
		if (_movingToSearchLocation)
		{
			if (_arrivedAtTargetSearchPosition)
			{
				...
				else if (_controller.GetAngleToLocalPosition(_searchPosition) < 5f)
				{
		...
		Vector3 localPos = _data.lastKnownPlayerLocation.localPosition + new Vector3(0f, 1.8f, 0f);
		bool flag2 = _sensors.CheckPositionOccluded(_controller.LocalToWorldPosition(localPos));
		float num = (_allowFocusBeam ? (_controller.GetFocusedLanternRange() - 3f) : (_controller.GetUnfocusedLanternRange() - 1f));
		bool flag3 = _data.lastKnownPlayerLocation.distance < _controller.GetUnfocusedLanternRange();
		if (_data.sensor.isPlayerIlluminatedByUs)
		{
			...
			{
				_controller.ChangeLanternFocus(flag3 ? 0f : 1f);
			}
		}
	...
	public override void OnArriveAtPosition()
	{
		...
		if (_searchNodesNearTarget)
		{
			int num = GenerateSpotlightList(_searchNode, _searchStartPosition - _searchNode.localPosition);
			if (num > 0)
			{
				_controller.SetLanternConcealed(concealed: false);
				_controller.ChangeLanternFocus(1f);
				_controller.FaceNodeList(s_nodesToSpotlight, num, TurnSpeed.MEDIUM, 1f);
			}
		}
		else if (_sawPlayerOccluded)
		{
		...
		{
			Debug.LogError("how did i get here\nthis is not my beautiful house\nthis is not my beautiful wife", _controller);
		}
	}
