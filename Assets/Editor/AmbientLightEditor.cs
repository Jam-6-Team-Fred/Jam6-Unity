...
namespace OW
{
	...
	public class AmbientLightEditor : LightEditor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			var target = (Light)this.target;
			if (target.type == LightType.Point && target.cookie != null)
			{
		...
		protected override void OnSceneGUI()
		{
			...
			if (target.type == LightType.Point && target.cookie != null)
			{
				...
				innerRadius = Handles.RadiusHandle(Quaternion.identity, target.transform.position, innerRadius);
		...
		private void DrawAmbientControls()
		{
			...
			newColor.r = Mathf.Sqrt(innerRadius / _range.floatValue);
			newColor.r = float.IsNaN(newColor.r) ? 0.0f : newColor.r;
			newColor.r = Mathf.Min(newColor.r, .9999f); // exactly 1 breaks it
