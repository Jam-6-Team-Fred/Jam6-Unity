...
namespace UnityEngine.PostProcessing
{
	public sealed class ScreenSpaceReflectionComponent : PostProcessingComponentCommandBuffer<ScreenSpaceReflectionModel>
	{
		private static class Uniforms
		{
			...
			internal static readonly int _SmoothnessCutoff = Shader.PropertyToID("_SmoothnessCutoff");
			...
			internal static readonly int _CameraReflectionsTexture = Shader.PropertyToID("_CameraReflectionsTexture");
		}
		...
		public override void PopulateCommandBuffer(CommandBuffer cb)
		{
			...
			material.SetFloat(Uniforms._SmoothnessCutoff, settings.reflection.smoothnessCutoff);
			...
			material.SetTexture(Uniforms._CameraReflectionsTexture, Texture2D.blackTexture);
			...
			cb.GetTemporaryRT(normalAndRoughnessTexture, -1, -1, 0, FilterMode.Point, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			cb.GetTemporaryRT(hitPointTexture, num2, num3, 0, FilterMode.Bilinear, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			for (int i = 0; i < 5; i++)
			{
				cb.GetTemporaryRT(m_ReflectionTextures[i], num2 >> i, num3 >> i, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			}
			cb.GetTemporaryRT(filteredReflections, num2, num3, 0, (!k_BilateralUpsample) ? FilterMode.Bilinear : FilterMode.Point, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			cb.GetTemporaryRT(finalReflectionTexture, num2, num3, 0, FilterMode.Point, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			...
			for (int j = 1; j < 5; j++)
			{
				...
				int num9 = j;
				cb.GetTemporaryRT(blurTexture, num2 >> num9, num3 >> num9, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
			...
			cb.GetTemporaryRT(tempTexture, camera.pixelWidth, camera.pixelHeight, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, context.useDynamicScale);
