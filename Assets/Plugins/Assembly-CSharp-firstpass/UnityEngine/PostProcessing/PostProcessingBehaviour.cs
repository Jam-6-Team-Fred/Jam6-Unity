...
namespace UnityEngine.PostProcessing
{
	...
	public class PostProcessingBehaviour : MonoBehaviour
	{
		...
		private SmaaComponent m_Smaa;

		private CustomGammaComponent m_CustomGamma;

		private EyeMaskComponent m_EyeMask;

		private PhospheneComponent m_Phosphenes;

		private VolumeOcclusionComponent m_VolumeOcclusion;
		...
		private void OnEnable()
		{
			...
			m_Smaa = AddComponent(new SmaaComponent());
			m_CustomGamma = AddComponent(new CustomGammaComponent());
			m_EyeMask = AddComponent(new EyeMaskComponent());
			m_Phosphenes = AddComponent(new PhospheneComponent());
			m_VolumeOcclusion = AddComponent(new VolumeOcclusionComponent());
		...
		private void OnPreCull()
		{
			...
			m_Smaa.Init(postProcessingContext, profile.antialiasing);
			m_CustomGamma.Init(postProcessingContext, profile.customGamma);
			m_EyeMask.Init(postProcessingContext, profile.eyeMask);
			m_Phosphenes.Init(postProcessingContext, profile.phosphenes);
			m_VolumeOcclusion.Init(postProcessingContext, profile.volumeOcclusion);
		...
		private void OnPreRender()
		{
			if (!(profile == null))
			{
				...
				TryExecuteCommandBuffer(m_VolumeOcclusion);
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (active2)
			{
				...
				m_Smaa.Render(renderTexture, renderTexture2);
				renderTexture = renderTexture2;
			}
			else if (flag2)
			{
				RenderTexture renderTexture3 = m_RenderTextureFactory.Get(renderTexture);
				m_Taa.Render(renderTexture, renderTexture3);
			...
			{
				flag = true;
				m_DepthOfField.Prepare(renderTexture, material, flag2);
			}
			if (m_Bloom.active)
			{
				flag = true;
			...
			flag |= TryPrepareUberImageEffect(m_EyeMask, material);
			flag |= TryPrepareUberImageEffect(m_Phosphenes, material);
			...
			if (active)
			{
				...
				TryPrepareUberImageEffect(m_CustomGamma, material2);
				if (flag)
				{
					RenderTexture renderTexture4 = m_RenderTextureFactory.Get(renderTexture);
					Graphics.Blit(renderTexture, renderTexture4, material, 0);
			...
			else
			{
				...
				flag |= TryPrepareUberImageEffect(m_CustomGamma, material);
				if (flag)
				{
